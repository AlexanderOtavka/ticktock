...
Run `npm install && bower install` to install the web app dependencies.
...
--appidentity_private_key_path service-account-secret-key.pem` to start the 
API server.  You will need a secret key file, but it's a secret, so keep it 
secret if I send one to you.  Secrecy is paramount!

Then run `gulp serve` to start serving the web app and to proxy the API 
through localhost:5000, *OR* run `gulp` to build the web app, which will then
serve on localhost:8080 with the API.  Note: the former uses browsersync, and so
is better for development, the latter is a more accurate representation of 
the production version of the site, and so is better for final testing.
